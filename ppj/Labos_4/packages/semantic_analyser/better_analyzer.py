...
class Analyzer:
    ...
    local_scope: Dict[str, Dict[str, str]] = {}
    loopCounter: int = 0
    is_register_free: List[bool] = [True, True, True, True, True]
    in_register: Dict[str, str] = {}
    ...
    def start(self):
        ...
        self.file.writelines(["    MOVE 40000, R7\n", "    CALL F_MAIN\n", "    HALT\n", "\n"])
    ...
    def step(self, node: Node, types_stack: List[Dict[str, Union[Type, FunctionType]]]) -> bool:
        ...
        if node.value and node.name.split()[0] == "KR_INT":
            node.type = Type.INT
            return True
        if node.value and node.name.split()[0] in (
                'L_ZAGRADA', 'D_ZAGRADA', 'L_VIT_ZAGRADA', 'D_VIT_ZAGRADA', 'KR_RETURN', 'TOCKAZAREZ'):
            self.file.write("   RET\n")
            return True
        ...
        if node.production == ('<primarni_izraz>', ['ZNAK']):
            ...
            node.type = Type.CHAR
            node.l_expression = False
            node.value = str(ord(child_znak.name.split()[2].replace("'", "")))
        ...
        if node.production == ('<postfiks_izraz>', ['<primarni_izraz>']):
            ...
            node.value = child_primarni_izraz.value
            return True
        ...
        if node.production == ('<postfiks_izraz>', ['<postfiks_izraz>', 'L_ZAGRADA', 'D_ZAGRADA']):
            ...
            node.l_expression = False
            self.file.write("    CALL F_" + self.tree.find_node(node.children[0]).value.upper() + "\n")
            return True
        if node.production == (
            ...
            for argument in child_lista_argumenata.value.split(","):
                self.file.write("    MOVE " + hex(int(argument)).replace("x", "") + ", R0\n")
                self.file.write("    PUSH R0\n")

            self.file.write("    CALL F_" + child_postfiks_izraz.value.upper() + "\n")
            self.file.write("    PUSH R6\n")
            node.value = "f"
            return True
        if node.production == ('<postfiks_izraz>', ['<postfiks_izraz>', 'OP_INC']):
            ...
            in_function = self.tree.find_node(self.tree.find_node(self.isinfunction(node)).children[1]).name.split()[2]
            variables = self.local_scope.get(in_function)
            if variables is not None:
                variables[child_postfiks_izraz.value] = str(int(variables[child_postfiks_izraz.value]) + 1)

            self.file.write("    ADD R0, 1, R0\n")
            node.value = "L_" + child_postfiks_izraz.value
            return True
        ...
        if node.production == ('<lista_argumenata>', ['<izraz_pridruzivanja>']):
            ...
            node.value = child_izraz_pridruzivanja.value
            return True
        if node.production == ('<lista_argumenata>', ['<lista_argumenata>', 'ZAREZ', '<izraz_pridruzivanja>']):
            ...
            node.value = child_lista_argumenata.value + ',' + child_izraz_pridruzivanja.value
            return True
        if node.production == ('<unarni_izraz>', ['<postfiks_izraz>']):
            ...
            node.value = child_postfiks_izraz.value
            return True
        if node.production == ('<unarni_izraz>', ['OP_INC', '<unarni_izraz>']):
            ...
            variables = self.local_scope.get(in_function)
            if variables is not None:
                variables[child_unarni_izraz.value] = str(int(variables[child_unarni_izraz.value]) + 1)
        ...
        if node.production == ('<unarni_izraz>', ['<unarni_operator>', '<cast_izraz>']):
            ...
            child_unarni_operator = self.get_node(node.children[0])
            self.step(child_unarni_operator, types_stack)
            ...
            node.type = Type.INT
            node.l_expression = False
            node.value = child_unarni_operator.value + child_cast_izraz.value
            return True
        ...
        if node.production == ('<cast_izraz>', ['<unarni_izraz>']):
            ...
            node.value = child_unarni_izraz.value
            return True
        if node.production == ('<cast_izraz>', ['L_ZAGRADA', '<ime_tipa>', 'D_ZAGRADA', '<cast_izraz>']):
            ...
            node.l_expression = False
            node.value = child_cast_izraz.value
            return True
        ...
        if node.production == ('<multiplikativni_izraz>', ['<cast_izraz>']):
            ...
            node.value = child_multiplikativni_izraz.value
            return True
        if node.production == ('<multiplikativni_izraz>', ['<multiplikativni_izraz>', 'OP_PUTA', '<cast_izraz>']):
            ...
            if not has_implicit_cast(child_multiplikativni_izraz.type, Type.INT):
                raise SemanticError(node)
            self.file.write("    MOVE " + hex(int(child_multiplikativni_izraz.value)).replace("x", "") + ", R0\n")
            self.file.write("    PUSH R0\n")
            ...
            if not has_implicit_cast(child_cast_izraz.type, Type.INT):
                raise SemanticError(node)
            self.file.write("    MOVE " + hex(int(child_cast_izraz.value)).replace("x", "") + ", R0\n")
            self.file.write("    PUSH R0\n")
            node.type = Type.INT
            node.l_expression = False

            self.file.writelines(["    POP R0\n", "    POP R1\n", "    MOVE 0, R2\n", "\n"])
            self.file.writelines(["M_LOOP" + self.loopCounter.__str__() + "  CMP R1, 1\n"])
            self.file.writelines(["    JP_N END_" + self.loopCounter.__str__() + "\n", "    ADD R2, R0, R2\n"])
            self.file.writelines(["    SUB R1, 1, R1\n", "    JP M_LOOP" + self.loopCounter.__str__() + "\n", "\n"])
            self.file.writelines(["END_" + self.loopCounter.__str__() + "\n"])
            self.file.writelines(["    PUSH R2\n"])
            self.loopCounter += 1
            # node.value = child_multiplikativni_izraz.value * child_cast_izraz.value
            node.value = "R2"
            return True
        if node.production == ('<multiplikativni_izraz>', ['<multiplikativni_izraz>', 'OP_DIJELI', '<cast_izraz>']):
            ...
            self.file.writelines(["    POP R0\n", "    POP R1\n", "    MOVE 0, R2\n", "\n"])
            self.file.writelines(["M_LOOP" + self.loopCounter.__str__() + "  CMP R1, R0\n"])
            self.file.writelines(["    JP_N END_" + self.loopCounter.__str__() + "\n", "    SUB R1, R0, R1\n"])
            self.file.writelines(["    ADD R2, 1, R2\n", "    JP M_LOOP" + self.loopCounter.__str__() + "\n", "\n"])
            self.file.writelines(["END_" + self.loopCounter.__str__() + "\n"])
            self.file.writelines(["    PUSH R2\n"])
            self.loopCounter += 1
            node.value = "R2"
        ...
        if node.production == ('<aditivni_izraz>', ['<aditivni_izraz>', 'PLUS', '<multiplikativni_izraz>']):
            ...
            if not has_implicit_cast(child_aditivni_izraz.type, Type.INT):
                raise SemanticError(node)
            if child_aditivni_izraz.value.isdigit():
                self.file.write("    MOVE " + hex(int(child_aditivni_izraz.value)).replace("x", "") + ", R0\n")
                self.file.write("    PUSH R0\n")
            elif self.global_variables.get(child_aditivni_izraz.value) is not None:
                self.file.write("    LOAD R0, (G_" + child_aditivni_izraz.value.upper() + ")\n")
                self.file.write("    PUSH R0\n")
            else:
                self.file.write("    ;Varijabla je na stogu\n")
            ...
            if not has_implicit_cast(child_multiplikativni_izraz.type, Type.INT):
                raise SemanticError(node)
            if child_multiplikativni_izraz.value.isdigit():
                self.file.write("    MOVE " + hex(int(child_multiplikativni_izraz.value)).replace("x", "") + ", R0\n")
                self.file.write("    PUSH R0\n")
            elif self.global_variables.get(child_multiplikativni_izraz.value) is not None:
                self.file.write("    LOAD R0, (G_" + child_multiplikativni_izraz.value.upper() + ")\n")
                self.file.write("    PUSH R0\n")
            else:
                self.file.write("    ;Varijabla je na stogu\n")
        ...
        if node.production == ('<aditivni_izraz>', ['<aditivni_izraz>', 'MINUS', '<multiplikativni_izraz>']):
            ...
            if not has_implicit_cast(child_aditivni_izraz.type, Type.INT):
                raise SemanticError(node)
            if child_aditivni_izraz.value.isdigit():
                self.file.write("      " + hex(int(child_aditivni_izraz.value)).replace("x", "") + ", R0\n")
                self.file.write("    PUSH R0\n")
            elif self.global_variables.get(child_aditivni_izraz.value) is not None:
                self.file.write("    LOAD R0, (G_" + child_aditivni_izraz.value.upper() + ")\n")
                self.file.write("    PUSH R0\n")
        ...
        if node.production == ('<odnosni_izraz>', ['<aditivni_izraz>']):
            ...
            node.value = child_aditivni_izraz.value
            return True
        ...
        if node.production == ('<jednakosni_izraz>', ['<odnosni_izraz>']):
            ...
            node.value = child_odnosni_izraz.value
            return True
        ...
        if node.production == ('<bin_i_izraz>', ['<jednakosni_izraz>']):
            ...
            node.value = jednakosni_izraz.value
            return True
        if node.production == ('<bin_i_izraz>', ['<bin_i_izraz>', 'OP_BIN_I', '<jednakosni_izraz>']):
            ...
            if not has_implicit_cast(child_bin_i_izraz.type, Type.INT):
                raise SemanticError(node)
            if child_bin_i_izraz.value.isdigit():
                self.file.write("    MOVE " + hex(int(child_bin_i_izraz.value)).replace("x", "") + ", R0\n")
                self.file.write("    PUSH R0\n")
            elif self.global_variables.get(child_bin_i_izraz.value) is not None:
                self.file.write("    LOAD R0, (G_" + child_bin_i_izraz.value.upper() + ")\n")
                self.file.write("    PUSH R0\n")
            else:
                self.file.write("    ;Varijabla je na stogu\n")
            ...
            if not has_implicit_cast(child_jednakosni_izraz.type, Type.INT):
                raise SemanticError(node)
            if child_jednakosni_izraz.value.isdigit():
                self.file.write("    MOVE " + hex(int(child_jednakosni_izraz.value)).replace("x", "") + ", R0\n")
                self.file.write("    PUSH R0\n")
            elif self.global_variables.get(child_jednakosni_izraz.value) is not None:
                self.file.write("    LOAD R0, (G_" + child_jednakosni_izraz.value.upper() + ")\n")
                self.file.write("    PUSH R0\n")
            else:
                self.file.write("    ;Varijabla je na stogu\n")
        ...
        if node.production == ('<bin_xili_izraz>', ['<bin_i_izraz>']):
            ...
            node.value = child_bin_i_izraz.value
            return True
        if node.production == ('<bin_xili_izraz>', ['<bin_xili_izraz>', 'OP_BIN_XILI', '<bin_i_izraz>']):
            ...
            scope_function = \
                self.tree.find_node(self.tree.find_node(self.isinfunction(node)).children[1]).name.split()[2]
            if child_bin_xili_izraz.value.isdigit():
                self.file.write("    MOVE " + hex(int(child_bin_xili_izraz.value)).replace("x", "") + ", R0\n")
                self.file.write("    PUSH R0\n")
            elif self.global_variables.get(child_bin_xili_izraz.value) is not None:
                self.file.write("    LOAD R0, (G_" + child_bin_xili_izraz.value.upper() + ")\n")
                self.file.write("    PUSH R0\n")
            elif self.local_scope.get(scope_function).get(child_bin_xili_izraz.value) is not None:
                self.file.write("    MOVE " + hex(
                    int(self.local_scope.get(scope_function).get(child_bin_xili_izraz.value))).replace("x",
                                                                                                       "") + ", R0\n")
                self.file.write("    PUSH R0\n")
            else:
                self.file.write("    ;Varijabla je na stogu\n")
            ...
            if child_bin_i_izraz.value.isdigit():
                ...
                self.file.write("    LOAD R0, (G_" + child_bin_i_izraz.value.upper() + ")\n")
                self.file.write("    PUSH R0\n")
            elif self.local_scope.get(scope_function).get(child_bin_i_izraz.value) is not None:
                self.file.write("    MOVE " + hex(
                    int(self.local_scope.get(scope_function).get(child_bin_i_izraz.value))).replace("x",
                                                                                                    "") + ", R0\n")
            else:
                self.file.write("    ;Varijabla je na stogu\n")
            node.type = Type.INT
        ...
        if node.production == ('<bin_ili_izraz>', ['<bin_xili_izraz>']):
            ...
            node.value = child_bin_xili_izraz.value
            return True
        if node.production == ('<bin_ili_izraz>', ['<bin_ili_izraz>', 'OP_BIN_ILI', '<bin_xili_izraz>']):
            ...
            if child_bin_ili_izraz.value.isdigit():
                self.file.write("    MOVE " + hex(int(child_bin_ili_izraz.value)).replace("x", "") + ", R0\n")
                self.file.write("    PUSH R0\n")
            elif self.global_variables.get(child_bin_ili_izraz.value) is not None:
                self.file.write("    LOAD R0, (G_" + child_bin_ili_izraz.value.upper() + ")\n")
                self.file.write("    PUSH R0\n")
            else:
                self.file.write("    ;Varijabla je na stogu\n")
        ...
        if node.production == ('<log_i_izraz>', ['<bin_ili_izraz>']):
            ...
            node.value = child_bin_ili_izraz.value
            return True
        ...
        if node.production == ('<log_ili_izraz>', ['<log_i_izraz>']):
            ...
            node.value = child_log_i_izraz.value
            return True
        ...
        if node.production == ('<izraz_pridruzivanja>', ['<log_ili_izraz>']):
            ...
            node.value = child_log_ili_izraz.value
            return True
        ...
        if node.production == ('<naredba_skoka>', ['KR_RETURN', 'TOCKAZAREZ']):
            # if self.tree.find_node(self.isinfunction(node)).children[1].name.split()[2] != 'main':
            # self.file.write("   MOVE R5,R7")
            if self.tree.find_node(self.tree.find_node(self.isinfunction(node)).children[1]).name.split()[2] != 'main':
                self.file.write("    PUSH R5\n")
            self.file.write("   RET\n")
            parent = self.get_node(node.parent)
            while parent is not None:
        ...
        if node.production == ('<naredba_skoka>', ['KR_RETURN', '<izraz>', 'TOCKAZAREZ']):
            ...
            if self.local_scope.get(scope_function).get(child_izraz.value) is not None:
                self.file.write("    MOVE " + hex(
                    int(self.local_scope.get(scope_function).get(child_izraz.value))).replace("x",
                                                                                              "") + ", R6\n")
            elif self.global_variables.get(child_izraz.value):
                self.file.write("   LOAD R6, (" + self.global_variables.get(child_izraz.value) + ")\n")
            elif child_izraz.value.find("-") != -1:
                value = int(child_izraz.value[1:])
                value = hex(value).replace("x", "")
                self.file.write("    MOVE -" + value + ", R6\n")
            elif child_izraz.value.find("~") != -1:
                value = int(child_izraz.value[1:])
                value = hex(value).replace("x", "")
                if value.__len__() >= 6:
                    self.file.write("    MOVE " + value[0: len(value) - 4] + ", R6\n")
                    self.file.write("    SHL R6, 010, R6\n")
                    self.file.write("    ADD R6, 0" + value[len(value) - 3:] + ", R6\n")
                else:
                    self.file.write("    MOVE " + value + ", R6\n")
                self.file.write("    MOVE 0ffff, R5\n")
                self.file.write("    SHL R5, 010, R5\n")
                self.file.write("    ADD R5, 0fff, R5\n")
                self.file.write("    XOR R6, R5, R6\n")
            elif child_izraz.value.find("!") != -1:
                value = int(child_izraz.value[1:])
                if value == 0:
                    self.file.write("    MOVE 1, R6\n")
                else:
                    self.file.write("    MOVE 0, R6\n")
            elif child_izraz.value.isdigit():
                value = hex(int(child_izraz.value)).replace("x", "")
                if value.__len__() >= 6:
                    self.file.write("    MOVE " + value[0: len(value) - 4] + ", R6\n")
                    self.file.write("    SHL R6, 010, R6\n")
                    self.file.write("    ADD R6, 0" + value[len(value) - 3:] + ", R6\n")
                    ...
                    self.file.write("    MOVE " + value + ", R6\n")
            elif child_izraz.value.find("R") != -1:
                self.file.write("    POP " + child_izraz.value + "\n")
                self.file.write("    MOVE " + str(child_izraz.value) + ", R6\n")
            elif self.local_scope.get(in_function) is not None:
                # if self.local_scope.get(in_function).get(child_izraz.value) is not None:
                if self.local_scope.get(in_function).get(child_izraz.value) is not None:
                    # TODO
                    # self.local_scope.get(in_function).get(child_izraz.value)
                    self.file.write("    MOVE " + self.in_register.get(child_izraz.value) + ", R6\n")

            elif child_izraz.value.find(",") != -1:
                for argument in child_izraz.value.split(","):
                    value = hex(int(argument)).replace("x", "")
                    self.file.write("    MOVE " + value + ", R0\n")
                    self.file.write("    PUSH R0\n")
            else:
                self.file.write("    MOVE %D " + str(child_izraz.value) + ", R6\n")
        ...
        if node.production == ('<vanjska_deklaracija>', ['<deklaracija>']):
            ...
            if not self.step(child_deklaracija, types_stack):
                raise SemanticError(node)
            node.value = child_deklaracija.value
            return True
        ...
        if node.production == (
            ...
            self.file.writelines("F_" + str(child_IDN.name.split(" ")[2]).upper() + " ")
            if self.tree.find_node(node.children[1]).name.split()[2] != 'main':
                self.file.write("    POP R5\n")
            self.local_scope.update({child_IDN.name.split(" ")[2]: {}})
            if not self.step(child_ime_tipa, types_stack):
                raise SemanticError(node)
            ...
            if not self.step(child_slozena_naredba, types_stack + [{}]):
                raise SemanticError(node)
            node.value = self.tree.find_node(node.children[1]).name.split()[2]
            return True
        if node.production == ('<definicija_funkcije>',
            ...
            child_ime_tipa = self.get_node(node.children[0])
            child_IDN = self.get_node(node.children[1])
            self.file.write("F_" + str(child_IDN.name.split(" ")[2]).upper())
            if self.tree.find_node(node.children[1]).name.split()[2] != 'main':
                self.file.write("    POP R5\n")
            self.local_scope.update({child_IDN.name.split(" ")[2]: {}})
        ...
        if node.production == ('<lista_init_deklaratora>', ['<init_deklarator>']):
            ...
            if not self.step(child_init_deklarator, types_stack):
                raise SemanticError(node)
            node.value = child_init_deklarator.value
            return True
        ...
        if node.production == ('<init_deklarator>', ['<izravni_deklarator>']):
            ...
            if self.isinfunction(node) == 0:
                self.file.write("DW %D " + self.global_variables.get(child_izravni_deklarator.value))
                self.file.write("\n")
            else:
                self.file.write("    MOVE " + child_izravni_deklarator.value + ", R0")
            node.value = child_izravni_deklarator.value
        ...
        if node.production == ('<init_deklarator>', ['<izravni_deklarator>', 'OP_PRIDRUZI', '<inicijalizator>']):
            ...
            if child_izravni_deklarator.type == Type.INT or child_izravni_deklarator.type == Type.CHAR:
                ...
                raise SemanticError(node)
            ancestor_function = self.isinfunction(node)
            if ancestor_function > 0:
                ancestor = self.tree.find_node(ancestor_function)
                function_name = self.tree.find_node(ancestor.children[1]).name.split()[2]
                if self.local_scope.get(function_name) is None:
                    self.local_scope.update({function_name: {}})
                self.local_scope.get(function_name).update({child_izravni_deklarator.value: child_inicijalizator.value})
            else:
                self.local_scope.update({"global": {child_izravni_deklarator.value: child_inicijalizator.value}})

            if self.isinfunction(node) == 0:
                self.file.write("DW %D " + self.local_scope.get("global").get(child_izravni_deklarator.value))
                self.file.write("\n")
            else:
                self.in_register.update({child_izravni_deklarator.value: "R0"})
                self.file.write("    MOVE " + hex(int(child_inicijalizator.value)).replace("x", "") + ", R0\n")

            node.value = "L_" + child_izravni_deklarator.value
            return True
        if node.production == ('<izravni_deklarator>', ['IDN']):
            ...
            node.value = child_IDN.value
            if self.isinfunction(node) == 0:
                self.file.write("G_" + child_IDN.name.split()[2].upper() + "    ")
                self.global_variables.update({child_IDN.name.split()[2]: "G_" + child_IDN.name.split()[2].upper()})
            return True
    ...
    def isinfunction(self, node: Node):
        parent = self.tree.find_node(node.parent)
        while True:
            if parent.index == 1:
                return 0
            elif parent.name == "<definicija_funkcije>":
                return parent.index
            else:
