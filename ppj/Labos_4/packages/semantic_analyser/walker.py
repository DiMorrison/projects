...
from Labos_4.packages.models.node import Type, Node
from Labos_4.packages.models.tree import Tree
from Labos_4.packages.semantic_analyser.production_parser import ProductionParser
...
class Walker:
    ...
    def walk(self, root: Node):
        sign_table: List[Dict[str, Type]] = list()  # tablica znakova
        self.file.writelines(["MOVE 40000, R7\n", "CALL F_MAIN\n", "HALT\n", "\n"])
        self.step(root, sign_table)

    def leaf(self, node: Node, sign_table: List[Dict[str, Type]]):
        pass

    def step(self, node: Node, sign_table: List[Dict[str, Type]]):
        # print("Step at node: " + str(node.index))
        if node.name == "<prijevodna_jedinica>":
            for child_id in node.children:
                child = self.tree.find_node(child_id)
                if self.step(child, sign_table):
                    return False
        ...
        if node.production == ('<primarni_izraz>', ['IDN']):
            IDN = self.tree.find_node(node.children[0])
            # provjera izraza
            if not self.step(IDN, sign_table):
                raise node.production
            node.type = IDN.type
            node.l_expression = IDN.l_expression

            return True
        elif node.production == ('<primarni_izraz>', ['BROJ']):
            node.type = Type.INT
            node.l_expression = False
            # Provjera je li u rasponu int
            if not self.tree.find_node(node.children[0]).value in range(-2147483647, 2147483647):
                raise node.production
            return True
            ...
            if not self.step(izraz, sign_table):
                raise node.production
            ...
            if not self.step(postfiks_izraz, sign_table):
                raise node.production
            ...
            if not self.step(self.tree.find_node(node.children[0]), sign_table):
                raise node.production
            ...
            if not self.step(self.tree.find_node(node.children[0]), sign_table):
                raise node.production
            if not self.step(self.tree.find_node(node.children[2]), sign_table):
                raise node.production
            ...
            if not self.step(cast_izraz, sign_table):
                raise node.production
            ...
            if not self.step(unarni_izraz, sign_table):
                raise node.production
            ...
            if not self.step(ime_tipa, sign_table):
                raise node.production
            if not self.step(self.tree.find_node(node.children[3]), sign_table):
                raise node.production
            ...
            if not self.step(specifikator_tipa, sign_table):
                raise node.production
            ...
            if not self.step(multiplikativni_izraz, sign_table):
                raise node.production
            ...
            if not self.step(aditivni_izraz, sign_table):
                raise node.production
            ...
            if not self.step(odnosni_izraz, sign_table):
                raise node.production
            ...
            if not self.step(jednakosni_izraz, sign_table):
                raise node.production
            ...
            if not self.step(bin_i_izraz, sign_table):
                raise node.production
            ...
            if not self.step(bin_xili_izraz, sign_table):
                raise node.production
            ...
            if not self.step(bin_ili_izraz, sign_table):
                raise node.production
            ...
            if not self.step(log_i_izraz, sign_table):
                raise node.production
            ...
            if not self.step(log_ili_izraz, sign_table):
                raise node.production
            ...
            postfiks_izraz = self.tree.find_node(node.children[0])
            if (not self.step(postfiks_izraz, sign_table)) or (not postfiks_izraz.l_expression):
                raise node.production

            izraz_pridruzivanja = self.tree.find_node(node.children[2])
            if not self.step(izraz_pridruzivanja, sign_table):
                raise node.production
            ...
            if not self.step(lista_naredbi, sign_table):
                raise node.production
            return True
        elif node.production == (
            ...
            if not self.step(lista_deklaracija, sign_table):
                raise node.production
            ...
            if not self.step(naredba, sign_table):
                raise node.production
            return True
            ...
            if not self.step(naredba_1, sign_table):
                raise node.production
            ...
            if not self.step(izraz_naredba_1, sign_table):
                raise node.production

            izraz_naredba_2 = self.tree.find_node(node.children[3])
            if not self.step(izraz_naredba_2, sign_table):
                raise node.production
            ...
            if not self.step(vanjska_deklaracija, sign_table):
                raise node.production
            return True
            ...
            if not self.step(prijevodna_jedinica, sign_table):
                raise node.production
            ...
            return True
                                 ...
                                 ['<ime_tipa>', 'IDN', 'L_ZAGRADA', '<lista_parametara>', 'D_ZAGRADA',
                                  '<slozena_naredba>']):
            self.file.write(self.tree.find_node(node.children[1]).index)
            ime_tipa = self.tree.find_node(node.children[0])
            if not self.step(ime_tipa, sign_table):
                raise node.production
            if ime_tipa.type == Type.T and ime_tipa.const:
            ...
            if not self.step(lista_parametara, sign_table):
                raise node.production
            ...
            if not self.step(deklaracija_parametra, sign_table):
                raise node.production
            ...
            ime_tipa = self.tree.find_node(node.children[0])
            if (not self.step(ime_tipa, sign_table)) or (ime_tipa.type == Type.VOID):
                raise node.production
            node.type = ime_tipa.type
            ...
            if not self.step(lista_init_deklaratora, sign_table):
                raise node.production
            return True
